...
// Directory containing the Telugu playlist JSON files
const inputDir = path.join(__dirname, "../../00-extract/output/telugu/playlist");
const videoDetailsDir = path.join(__dirname, "../../00-extract/output/telugu/playlist-video-details");
const outputDir = path.join(__dirname, "../output/telugu");
...
// Function to read and process all Telugu playlist files
async function processAllTeluguVideos() {
  console.log("Starting to process all Telugu playlist files...");
  ...
  console.log(`Found ${files.length} Telugu playlist files to process`);
  ...
  // Write combined telugu-meta.json file
  const combinedOutputFile = path.join(path.dirname(outputDir), "telugu-meta.json");
...
// Run the script
processAllTeluguVideos().catch(console.error);
