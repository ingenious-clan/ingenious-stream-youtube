...
// Paths
const inputPath = path.join(__dirname, '../output/telugu-meta.json');
const outputDir = path.join(__dirname, '../../movies/language/telugu');
...
// Find and update Telugu entry, or create it if it doesn't exist
const teluguIndex = languageMeta.findIndex(lang => lang.slug === 'telugu');
if (teluguIndex !== -1) {
  languageMeta[teluguIndex].count = movies.length;
} else {
  languageMeta.push({
    name: 'Telugu',
    slug: 'telugu',
